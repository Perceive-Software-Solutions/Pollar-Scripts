...
module.exports = async (conn, data) => {
  ...
  for(notifSet of data.notificationsettings){ //TODO double check that 0=off, 1=follwoers, 2=everyone
    ...
    conn.query(`INSERT INTO NotificationSettings (userMainID, POSTS_LIKES, POSTS_NEW_TRUSTS, POSTS_FIRST_POSTS, POSTS_REPLIES, 
            ...
            VALUES (${uSet2uMain[notifSet.userSettingId]}, "${num2enum(notifSet.posts[likes])}", "${num2enum(notifSet.posts[newTrusts])}", "${num2enum(notifSet.posts[firstPosts])}", "${num2enum(notifSet.posts[replies])}", 
