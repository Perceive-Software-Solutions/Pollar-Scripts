...
CREATE TRIGGER defualt_date_story BEFORE INSERT ON Story 
FOR EACH ROW SET
    NEW.expireAt = IFNULL(NEW.expireAt, TIMESTAMPADD(DAY, 1, NOW()));
    
CREATE TRIGGER defualt_date_userDeactivated BEFORE INSERT ON UserDeactivated 
FOR EACH ROW SET
    NEW.PIT = IFNULL(NEW.PIT, TIMESTAMPADD(DAY, 1, NOW()));